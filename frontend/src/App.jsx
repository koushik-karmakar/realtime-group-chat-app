...
function App() {
  ...
  useEffect(() => {
    ...
    socket.on("group:status", (s) => { setGroupExists(s.hasHost); setHostUsername(s.hostUsername); });
    socket.on("users:update",  (list) => setUsers(list));
    socket.on("typing:update", (list) => setTypingUsers(list.filter((u) => u !== currentUser)));
    socket.on("join:request",  (req)  => setJoinRequests((p) => [...p, {
  ...
  useEffect(() => {
    ...
    return () => { socket.off("join:success"); socket.off("join:error"); };
  }, []);

  useEffect(() => {
    const socket = socketUsRef.current;
    socket.on("message:new", (msg) =>
      setMessages((p) => [...p, { ...msg, sender: msg.sender === socket.id ? "me" : "other" }])
    );
  ...
  const handleUsernameSubmit = (e) => {
    ...
    if (!user)              { setJoinError("Username is required"); return; }
    if (user.length < 3)   { setJoinError("Username must be at least 3 characters"); return; }
    if (!/^[a-zA-Z0-9]+$/.test(user)) { setJoinError("Letters and numbers only"); return; }
    setJoinError(""); setCurrentUser(user); setShowPasswordField(true);
    if (!groupExists) setIsUsernameSet(true);
  };

  const handleSendRequest = () => {
    if (!username.trim()) { setJoinError("Username is required"); return; }
    setJoinError(""); setIsUsernameSet(true); setCurrentUser(username.trim());
    socketUsRef.current.emit("joinWithPassword", { username: username.trim(), password: "" });
  };

  const handleJoinWithPassword = (e) => {
    e.preventDefault();
    if (isRejected || hasBeenRejectedRef.current) {
      setJoinError("You were rejected. Please refresh to try again."); return;
    }
    const user = username.trim(); const pass = password.trim();
    if (!user || !pass)    { setJoinError("Username and password required"); return; }
    if (user.length < 3)  { setJoinError("Username must be at least 3 characters"); return; }
    if (!/^[a-zA-Z0-9]+$/.test(user)) { setJoinError("Letters and numbers only"); return; }
    setJoinError(""); setIsUsernameSet(true); setCurrentUser(user);
    setIsRejected(false); hasBeenRejectedRef.current = false;
    socketUsRef.current.emit("joinWithPassword", { username: user, password: pass });
  };
  ...
  const handleBackToUsername = () => { setShowPasswordField(false); setPassword(""); setJoinError(""); };

  const handleTyping = (e) => {
    setNewMessage(e.target.value);
    if (!e.target.value.trim()) { socketUsRef.current.emit("stopTyping", currentUser); return; }
    socketUsRef.current.emit("typing", currentUser);
    clearTimeout(typingTimeout.current);
    typingTimeout.current = setTimeout(() => socketUsRef.current.emit("stopTyping", currentUser), 1500);
  };
  ...
  const handleSendMessage = (e) => {
    ...
    socketUsRef.current.emit("sendMessage", {
      ...
      sender: socketUsRef.current.id, username: currentUser,
      time: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
    });
    setNewMessage("");
  };
          ...
          <div className="rounded-2xl border border-white/[0.07] bg-[#0d1520] p-8 shadow-[0_32px_80px_rgba(0,0,0,0.65)]">
            ...
            <div className="mb-6 flex justify-center">
              <div className={`flex h-16 w-16 items-center justify-center rounded-2xl border ${
                ...
                {groupExists ? <Lock className="h-7 w-7" /> : <Shield className="h-7 w-7" />}
              </div>
            </div>
            ...
            <div className="mb-7 text-center">
              ...
              <p className="text-sm text-slate-500">
                {groupExists ? `Hosted by ${hostUsername}` : "You'll become the host"}
              </p>
            </div>

            {joinError && (
              <div className={`lobby-alert-in mb-5 flex items-start gap-2.5 rounded-xl border px-3.5 py-3 text-sm leading-relaxed ${
                isPending
              ...
              <form onSubmit={handleUsernameSubmit} className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    placeholder="e.g. Alex42" autoFocus
                  ...
                  />
                  <p className="text-[11px] text-slate-700">Letters &amp; numbers · min 3 chars</p>
                </div>
                <button type="submit"
              ...
              <form onSubmit={handleJoinWithPassword} className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    placeholder={groupExists ? "Enter to join directly…" : "Choose a group password"}
                    disabled={isPending || isRejected || hasBeenRejectedRef.current}
                    className="w-full rounded-xl border border-white/8 bg-[#111d2b] px-4 py-3 text-sm text-white placeholder-slate-700 outline-none transition-all focus:border-emerald-500/35 focus:ring-2 focus:ring-emerald-500/10 disabled:cursor-not-allowed disabled:opacity-40"
                  />
                  <p className="text-[11px] text-slate-700">
                    {groupExists ? "Leave empty to send a join request" : "Members will use this to enter"}
                  </p>
                </div>
                {groupExists && !password.trim() ? (
                  ...
                  <button type="submit"
                    ...
                    {groupExists ? "Join Room" : "Create Room"} <span className="btn-arrow">→</span>
                  </button>
                )}
                {(isRejected || hasBeenRejectedRef.current) && (
                  <button type="button" onClick={handleResetForm}
  ...
  return (
    <div className="flex w-full flex-col overflow-hidden bg-[#07090d]" style={{ height: "100dvh" }}>

      {isHost && hostAlerts.length > 0 && (
        <div className="fixed right-4 top-4 z-50 flex w-72 max-w-[calc(100vw-2rem)] flex-col gap-2">
          {hostAlerts.map((a) => (
            <div key={a.id} className="toast-in flex items-start gap-3 rounded-xl border border-sky-500/20 bg-[#0c1e30]/95 px-4 py-3.5 shadow-2xl backdrop-blur-xl">
              <AlertCircle className="mt-0.5 h-4 w-4 shrink-0 text-sky-400" />
              <div>
                <p className="text-xs font-semibold text-sky-300">{a.username} wants to join</p>
      ...
      {isHost && joinRequests.length > 0 && (
        <div className="fixed right-4 top-18 z-50 flex w-72 max-w-[calc(100vw-2rem)] flex-col gap-2.5">
          {joinRequests.map((req) => (
            <div key={req.id} className="toast-in rounded-xl border border-amber-500/20 bg-[#130f05]/95 p-4 shadow-2xl backdrop-blur-xl">
              <div className="mb-2.5 flex items-center justify-between">
                <div className="flex items-center gap-1.5">
                  <UserPlus className="h-3.5 w-3.5 text-amber-400" />
                  <span className="text-[10px] font-bold uppercase tracking-wider text-amber-400">Join Request</span>
                </div>
                <button onClick={() => handleDismissRequest(req.socketId)}
                  className="rounded p-0.5 text-slate-600 transition hover:text-slate-300">
                  <X className="h-3.5 w-3.5" />
                </button>
              </div>
              <p className="font-syne text-sm font-bold text-white">{req.username}</p>
      ...
      <header className="chat-header-sticky flex shrink-0 items-center justify-between border-b border-white/5 bg-[#0b1219] px-4 py-3">
        <div className="flex min-w-0 items-center gap-3">
          <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border ${
            ...
            {isHost ? <Shield className="h-4 w-4" /> : <Users className="h-4 w-4" />}
          </div>
          <div className="min-w-0">
            <h2 className="font-syne text-sm font-extrabold tracking-tight text-white">Group Room</h2>
            <div className="mt-0.5 flex items-center gap-1.5 text-[11px] text-slate-500">
              ...
              <span className="truncate font-medium text-slate-300">{currentUser}</span>
              {isHost && (
      ...
      <div className="chat-scroll chat-bg min-h-0 flex-1 overflow-y-auto px-3 py-5 sm:px-5">
        <div className="flex flex-col gap-1.5">
          {messages.map((msg) => {
            const isSystem = msg.sender === "system" || msg.username === "System";
            const isMe     = !isSystem && msg.sender === "me";
            return (
              <div key={msg.id}
                className={`msg-in flex ${
                  isSystem ? "justify-center py-1"
                  : isMe   ? "justify-end"
                  :          "justify-start"
                }`}>
                {isSystem ? (
                  ...
                  <div className={`max-w-[75%] rounded-2xl px-3.5 py-2.5 sm:max-w-sm lg:max-w-md ${
                    isMe
